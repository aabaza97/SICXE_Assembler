...
import static me.whothefuckis.manager.SysConverter.*;
import static me.whothefuckis.manager.Validator.Operand.*;
...
public final class ObjectCodeGenerator {
    ...
    private void generateTextRecord() {
        ...
        for (Instruction instruction : parsedInstructions) {
            ...
            if (isOperation(instruction.getMnemonic())) {
                objectCode = switch (OpTable.getOperation(instruction.getMnemonic()).getFormat()) {
                    case ONE -> generateFormatOne(instruction);
                    case TWO -> generateFormatTwo(instruction);
                    case THREE -> generateFormatThree(instruction);
                    case FOUR -> generateFormatFour(instruction);
                };
            } else if (instruction.getMnemonic().equals(DirTable.BYTE)) {
                String operand = instruction.getFirstOperand();
                if (isLiteral(operand)) {
                    String[] ocStream = Utility.splitAfter(parseDataOperand(operand), 6);
                    for (String oc : ocStream)
                        objectCodes.add(
                                new ObjectCode(extendLength(oc, Utility.ceilToEven(oc.length())),
                                        instruction.getAddress()));
                } else
                    objectCode = extendLength(Decimal.toHexadecimal(operand), Utility.ceilToEven(operand.length()));
            } else if (instruction.getMnemonic().equals(DirTable.WORD)) {
                String operand = instruction.getFirstOperand();
                if (isLiteral(operand))
    ...
    private String generateFormatOne(Instruction instruction) {
        String opcode = String.valueOf(OpTable.getOperation(instruction.getMnemonic()).getOpcode());
    ...
    private String getBPE(Instruction instruction) {
        char b, p, e;
        if (OpTable.getOperation(instruction.getMnemonic()).getFormat() == Format.FOUR) {
            b = '0';
            p = '0';
            e = '1';
            return String.valueOf(b) + p + e;
        } else {
            String operand = Utility.removeSpecialSymbol(instruction.getFirstOperand());
            e = '0';
